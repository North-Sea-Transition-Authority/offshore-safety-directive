...
class NominationRequestUpdateControllerTest extends AbstractNominationControllerTest {
  ...
  void updateWonsReferences_whenUserNotLoggedIn() throws Exception {
    mockMvc.perform(post(ReverseRouter.route(on(NominationRequestUpdateController.class)
        .requestUpdate(NOMINATION_ID, true, CaseProcessingActionIdentifier.REQUEST_UPDATE, null, null, null)))
        .with(csrf()))
