...
class NominationPortalReferenceControllerTest extends AbstractNominationControllerTest {
  ...
  @SecurityTest
  void updatePearsReferences_whenUserNotLoggedIn() throws Exception {
    mockMvc.perform(post(ReverseRouter.route(on(NominationPortalReferenceController.class)
        .updatePearsReferences(NOMINATION_ID, true, CaseProcessingActionIdentifier.PEARS_REFERENCES, null, null, null)))
        .with(csrf()))
  ...
  @SecurityTest
  void updatePearsReferences_whenUserIsNotTheNominationManager() throws Exception {
  ...
  @SecurityTest
  void updateWonsReferences_whenUserNotLoggedIn() throws Exception {
    mockMvc.perform(post(ReverseRouter.route(on(NominationPortalReferenceController.class)
        .updateWonsReferences(NOMINATION_ID, true, CaseProcessingActionIdentifier.WONS_REFERENCES, null, null, null)))
        .with(csrf()))
        .andExpect(status().is3xxRedirection())
        .andExpect(redirectionToLoginUrl());
  }
  ...
  @SecurityTest
  void updateWonsReferences_whenUserIsNotTheNominationManager() throws Exception {
