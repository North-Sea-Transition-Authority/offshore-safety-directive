...
class NomineeDetailControllerTest extends AbstractNominationControllerTest {
  ...
  @SecurityTest
  void getNomineeDetail_whenNotLoggedIn() throws Exception {
    mockMvc.perform(get(ReverseRouter.route(on(NomineeDetailController.class).getNomineeDetail(nominationId))))
        .andExpect(status().is3xxRedirection())
        .andExpect(redirectionToLoginUrl());
  }

  @SecurityTest
  void getNomineeDetail_whenNotMemberOfApplicantTeamWithDraftNominationRole() throws Exception {
  ...
  @SecurityTest
  void saveNomineeDetail_whenNotLoggedIn() throws Exception {
    mockMvc.perform(post(ReverseRouter.route(on(NomineeDetailController.class).saveNomineeDetail(nominationId, null, null)))
        .with(csrf()))
        .andExpect(status().is3xxRedirection())
        .andExpect(redirectionToLoginUrl());
  }
  ...
  @SecurityTest
  void saveNomineeDetail_whenNotMemberOfApplicantTeamWithDraftNominationRole() throws Exception {
  ...
  void saveNomineeDetail_whenValidForm_verifyMethodCalls() throws Exception {
    ...
    mockMvc.perform(post(ReverseRouter.route(on(NomineeDetailController.class).saveNomineeDetail(nominationId, form, null)))
        .with(csrf())
        .with(user(USER)))
        .andExpect(status().is3xxRedirection());

    verify(nomineeDetailFormService).validate(any(), any());
    verify(nominationDetailService, times(3)).getLatestNominationDetail(nominationId);
  ...
  void saveNomineeDetail_whenInvalidForm_verifyStatusIsOk() throws Exception {
    ...
    var modelAndView = mockMvc.perform(post(ReverseRouter.route(on(NomineeDetailController.class)
        .saveNomineeDetail(nominationId, form, null)))
        .with(csrf())
        .with(user(USER))
