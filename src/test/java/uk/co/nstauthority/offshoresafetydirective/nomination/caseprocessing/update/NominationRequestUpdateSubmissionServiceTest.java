...
class NominationRequestUpdateSubmissionServiceTest {
  ...
  void submit_verifyCalls() {
    ...
    var submitterView = EnergyPortalUserDtoTestUtil.Builder().build();

    when(nominationRoleService.getUsersInApplicantOrganisationTeamWithAnyRoleOf(
        NOMINATION_DETAIL,
        EnumSet.of(Role.NOMINATION_SUBMITTER)
    ))
    ...
    assertThat(mergedTemplateCaptor.getValue().getMailMergeFields())
        .containsExactlyInAnyOrder(
            ...
            new MailMergeField("RECIPIENT_IDENTIFIER", submitterView.forename())
        );
  }
  ...
  void submit_whenGetTemplateThrowsException_thenNoEmailSent() {
    ...
    verify(caseEventService).createUpdateRequestEvent(NOMINATION_DETAIL, reason);
    verify(emailService, never()).sendEmail(any(), any(), any());
    verifyNoInteractions(emailUrlGenerationService);
  }
}