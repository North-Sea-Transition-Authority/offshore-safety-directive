...
class UpdateRequestInterceptorTest extends AbstractControllerTest {
  ...
  void preHandle_whenEndpointNotUsingSupportedAnnotation_thenOkWithNoInteractions() throws Exception {
    ...
    var route = ReverseRouter.route(
        on(UpdateRequestInterceptorTestController.class)
  ...
  void preHandle_whenNoNominationIdRequestParam_thenOkWithNoInteractions() throws Exception {
    var route = ReverseRouter.route(on(UpdateRequestInterceptorTestController.class).endpointWithoutNominationId());
    mockMvc.perform(
        get(route)
            .with(user(ServiceUserDetailTestUtil.Builder().build()))
    )
  ...
  void preHandle_whenEndpointHasBothSupportedAnnotations_thenInternalServerError() throws Exception {
    var route = ReverseRouter.route(on(UpdateRequestInterceptorTestController.class).hasBothSupportedAnnotations(
        nominationId
    ));
    mockMvc.perform(
        get(route)
  ...
  void preHandle_whenNoUpdateRequestRequiredAndUpdateRequested_thenForbidden() throws Exception {
    var route = ReverseRouter.route(on(UpdateRequestInterceptorTestController.class).hasNoUpdateRequest(
        nominationId
    ));
  ...
  void preHandle_whenNoUpdateRequestRequiredAndUpdateRequested_thenOk() throws Exception {
    var route = ReverseRouter.route(
        on(UpdateRequestInterceptorTestController.class).hasNoUpdateRequest(nominationId)
    );
