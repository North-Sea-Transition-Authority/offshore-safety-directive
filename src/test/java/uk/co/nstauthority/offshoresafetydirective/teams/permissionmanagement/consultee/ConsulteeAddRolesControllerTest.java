...
@ContextConfiguration(classes = {
    ConsulteeAddRolesController.class,
    ConsulteeTeamMemberRolesValidator.class
})
class ConsulteeAddRolesControllerTest extends AbstractControllerTest {
  ...
  void renderAddTeamMemberRoles_whenUserIsNotLoggedIn_thenRedirectedToLogin() throws Exception {
    ...
    when(energyPortalUserService.findByWuaId(webUserAccountIdToAdd, ConsulteeAddRolesController.ROLES_TO_ADD_PURPOSE))
  ...
  void renderAddTeamMemberRoles_whenAccessManager_thenOk() throws Exception {
    ...
    when(energyPortalUserService.findByWuaId(teamMember.wuaId(), ConsulteeAddRolesController.ROLES_TO_ADD_PURPOSE))
  ...
  void renderAddTeamMemberRoles_whenNoTeamFound_thenNotFound() throws Exception {
    ...
    when(teamService.getTeam(teamId, ConsulteeAddRolesController.TEAM_TYPE)).thenReturn(Optional.empty());
  ...
  void renderAddTeamMemberRoles_assertModelProperties() throws Exception {
    ...
    mockMvc.perform(
        ...
        .andExpect(model().attribute(
            "backLinkUrl",
            ReverseRouter.route(on(ConsulteeAddMemberController.class)
                .renderAddTeamMember(teamId))
        ))
        .andExpect(model().attributeExists("form"))
        .andExpect(model().attribute("roles", getDisplayableConsulteeRoles()));
  }
  ...
  private Map<String, String> getDisplayableConsulteeRoles() {
    return Arrays.stream(ConsulteeTeamRole.values())
        .sorted(Comparator.comparing(ConsulteeTeamRole::getDisplayOrder))
        .collect(StreamUtil.toLinkedHashMap(
            ConsulteeTeamRole::name,
