...
@ContextConfiguration(classes = {
    ConsulteeAddMemberController.class
})
class ConsulteeAddMemberControllerTest extends AbstractControllerTest {
  ...
  void renderAddTeamMember_whenUserIsNotLoggedIn_thenRedirectedToLogin() throws Exception {
    ...
    when(teamService.getTeam(teamId, ConsulteeAddRolesController.TEAM_TYPE)).thenReturn(Optional.of(team));

    when(energyPortalUserService.findByWuaId(webUserAccountIdToAdd, ConsulteeAddMemberController.USER_TO_ADD_PURPOSE))
        .thenReturn(Optional.of(EnergyPortalUserDtoTestUtil.Builder().build()));

    mockMvc.perform(
            get(ReverseRouter.route(on(ConsulteeAddMemberController.class)
  ...
  void renderAddTeamMember_whenAccessManager_thenOk() throws Exception {
    ...
    mockMvc.perform(
            get(ReverseRouter.route(on(ConsulteeAddMemberController.class).renderAddTeamMember(teamId)))
                .with(user(user)))
  ...
  void renderAddTeamMember_whenNoTeamFound_thenNotFound() throws Exception {
    ...
    when(teamService.getTeam(teamId, ConsulteeAddMemberController.TEAM_TYPE)).thenReturn(Optional.empty());
  ...
  @Test
  void renderAddTeamMember_whenTeamIdIsNotConsulteeTeam_thenIsForbidden() throws Exception {
  ...
  void renderAddTeamMember_assertModelProperties() throws Exception {
    ...
    mockMvc.perform(
        ...
        .andExpect(model().attribute(
            "submitUrl",
            ReverseRouter.route(on(ConsulteeAddMemberController.class).addMemberToTeamSubmission(teamId, null, null))
        ));
  }
  ...
  @Test
  void addMemberToTeamSubmission_whenTeamIdNotConsulteeType_thenForbidden() throws Exception {
  ...
  void addMemberToTeamSubmission_whenValidForm_thenUserTakenToRolesSelection() throws Exception {
    ...
    when(energyPortalUserService.findUserByUsername(username, ConsulteeAddMemberController.USER_TO_ADD_PURPOSE))
